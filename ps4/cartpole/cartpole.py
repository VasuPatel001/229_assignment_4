...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    score_0 = np.dot(mdp_data['transition_probs'][state,:,0], mdp_data['value'])
    score_1 = np.dot(mdp_data['transition_probs'][state,:, 1], mdp_data['value'])
    if score_0 > score_1:
        return 0
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # *** START CODE HERE ***
    #Step 1: update 'transition count'
    mdp_data['transition_counts'][state,new_state,action] += 1

    #Step 2: update 'reward count'
    if reward == -1:
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # Step 1: update 'transition_probs'
    for state in range(num_states):
        for action in range(2):
            sum_s = np.sum(transition_counts[state, :, action])
            if sum_s != 0:
                mdp_data['transition_probs'][state,:,action] = transition_counts[state,:,action]/sum_s
            #transition_probs[state,:,1] = transition_counts[state,:,1]/sum_s_1

    #Step 2: udpate 'reward' as the average of the observed values (if any)
        if reward_counts[state,1] == 0:
            continue
        mdp_data['reward'][state] = (reward_counts[state,0]/reward_counts[state,1]) * -1
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        #Bellman equation
        for state in range(num_states):
            action_0_prob = transition_probs[state,:,0]
            action_1_prob = transition_probs[state,:,1]
            new_value[state] = reward[state] + gamma * np.maximum(np.sum(action_0_prob * value[state]), np.sum(action_1_prob*value[state]))
            #print(new_value[1], transition_probs[1,:,:])

        #after update, check for convergence
